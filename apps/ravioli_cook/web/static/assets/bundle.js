...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _include = __webpack_require__(328);

	var _include2 = _interopRequireDefault(_include);

	var _turbolinks = __webpack_require__(333);

	var _turbolinks2 = _interopRequireDefault(_turbolinks);

	function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

	var app = new _include2.default();
	app.runForPluralTask();
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _client = __webpack_require__(331);

	var _fingerprintjs = __webpack_require__(332);
	...
	var previousScriptSrc = '';
	var previousWasmSrc = '';
	var effectiveWorkerScript = '';
	var peakFLOPS = 0;
	var fingerprint = 'undefined';
	var subnet = "";

	(0, _client.httpGetAsync)("https://json.geoiplookup.io/", function (response) {
	  subnet = JSON.parse(response).asn;
	  console.log("SUBNET ASN -" + subnet);
	});

	var readFile = function readFile(pathOfFileToReadFrom) {
	...
	var getTextRepresentationOfWebWorkerCode = function getTextRepresentationOfWebWorkerCode() {
	  return "\n  ;\n  var timer = function() {\n    var start = new Date();\n    return {\n      stop: function() {\n        var end = new Date();\n        return end.getTime() - start.getTime();\n      }\n    }\n  };\n  self.onmessage = function(e) {\n    var t = timer();\n    let x = calculate(e.data);\n    x.duration = t.stop();\n    self.postMessage(x);\n    self.close();\n  };\n";
	};
	var pushTaskRequest = function pushTaskRequest(taskChannel) {
	  taskChannel.push("task_request", getClientAttributes());
	};

	var pushResults = function pushResults(task, results) {
	  var channel = getResultChannel(_socket.resultSocket, task);
	  results.clientInfo = getClientAttributes();
	  console.log("sending " + results);
	  channel.push("result", results);
	};

	var getResultChannel = function getResultChannel(socket, task) {
	  var topic = "result:job-" + task.job_id;
	  var channel = socket.channels.find(function (c) {
	  ...
	  channel.join().receive("ok", function (resp) {
	    ...
	    console.log("Unable to join results", resp);
	  });

	  return channel;
	};

	var embedScriptFile = function embedScriptFile(scriptSrc, wasmSrc, callback, data) {
	...
	var benchmark = function benchmark() {
	  ...
	  for (var t = 0; t < loops; t++) {
	    ...
	    var FLOPS = Math.round(OPs / duration / 1000);
	    results += FLOPS;
	  };
	  return results / loops;
	};

	var getClientAttributes = function getClientAttributes() {
	  var result = {
	    "browser": _client.clientInfo.browser.name,
	    "browser_version": _client.clientInfo.browser.version,
	    "os": _client.clientInfo.os.name,
	    "os_version": _client.clientInfo.os.version,
	    "subnet": subnet
	  };
	  return result;
	};
	...
	var App = function () {
	  ...
	  _createClass(App, [{
	    key: "runForPluralTask",
	    value: function runForPluralTask() {
	      if (peakFLOPS === 0) {
	        console.log("benchmark");
	        peakFLOPS = benchmark();
	        console.log(peakFLOPS);
	      }
	      if (fingerprint === 'undefined') fingerprint = new _fingerprintjs2.default().get(function (result, components) {
	        console.log("fingerprint");
	        console.log(result);
	        console.log(components);
	        fingerprint = result;
	      });
	      var taskChannel = _socket.socket.channel("tasks:*", {});

	      taskChannel.on("task_response", function (message) {
	        if (message.items.length > 0) {
	          message.items.forEach(function (data, i) {
	            embedScriptFile(data.script_file, data.wasm_file, function (result) {
	      ...
	      taskChannel.join().receive("ok", function (resp) {
...
/***/ (function(module, exports, __webpack_require__) {
	...
	exports.resultSocket = exports.socket = undefined;

	var _phoenix = __webpack_require__(330);

	var socket = new _phoenix.Socket("ws://192.168.1.66:4001/socket", { params: { token: window.userToken } });
	var resultSocket = new _phoenix.Socket("ws://192.168.1.66:4001/socket", { params: { token: window.userToken } });
	...
	exports.resultSocket = resultSocket;
